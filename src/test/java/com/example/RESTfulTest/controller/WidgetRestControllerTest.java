...
class WidgetRestControllerTest {
    ...
    void succesPutTestWidget() throws Exception {
        Widget widgetToUpdate = new Widget(1l, "New Widget", "Description1", 1);
        Widget widgetToReturn = new Widget(1l, "Update widget2", "Description 2", 1);
        ...
        mockMvc.perform(put("/rest/widget/{id}", 1L)
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.name", is("Update widget2")))
    ...
    void notFoundPutTest() throws Exception {
        Widget widgetToUpdate = new Widget(1l, "Widget updated", "Description1", 1);
        doReturn(Optional.empty()).when(service).findById(1l);
